...
onMounted(() => {
    editor.value = new Editor({
        ...
        editorProps: {
            attributes: {
                class: 'prose prose-sm prose-invert focus:outline-none min-h-[150px] w-full',
                dir: 'rtl'
            }
        }
    })
...
<template>
  <div v-if="store.isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" dir="rtl">
    <div class="bg-[#1e1e1e] border border-gray-700 rounded-lg shadow-2xl w-[90%] max-w-lg overflow-hidden flex flex-col max-h-[80vh]">
        <!-- Header -->
        <div class="flex items-center justify-between px-4 py-3 border-b border-gray-700 bg-[#252525]">
            <h3 class="text-white font-bold text-sm">تحرير الحاشية</h3>
            <button @click="store.closeEditor" class="text-gray-400 hover:text-white transition-colors">
                ✕
            </button>
        </div>
        ...
        <!-- Footer -->
        <div class="px-4 py-3 border-t border-gray-700 bg-[#252525] flex justify-end gap-2">
            <button @click="store.closeEditor" class="px-4 py-1.5 rounded text-xs text-gray-300 hover:bg-gray-700">
                إلغاء
            </button>
