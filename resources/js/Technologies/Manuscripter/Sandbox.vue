...
<template>
    ...
    -->
    <div class="h-screen w-screen bg-stone-900 relative overflow-hidden font-ui text-gray-800" dir="rtl">
        ...
        <main class="absolute inset-0 z-0 bg-[#0c0c0c] overflow-hidden">
            ...
            -->
            <div v-if="viewMode === 'default'" class="w-full h-full overflow-y-auto custom-scrollbar">
                <div class="max-w-5xl mx-auto flex flex-col gap-2 py-8 pt-20 pb-20 px-4">
                    ...
                    >
                        ...
                        <div class="h-full aspect-[2/3] bg-black rounded overflow-hidden relative shadow-sm">
                            <img :src="getPageUrl(i, displayedVersions[0])" loading="lazy" class="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
                        </div>
                        ...
                        <div class="flex items-center gap-2 px-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button class="p-2 rounded hover:bg-white/10 text-white/60 hover:text-white" title="عرض">
            ...
            -->
            <div v-else-if="viewMode === 'grid'" class="w-full h-full overflow-y-auto custom-scrollbar px-4">
                <div class="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 gap-2 pt-20 pb-20">
                    <div v-for="i in 100" :key="i" 
                        ...
                        @dblclick="shotNumber = i; viewMode = 'list'; scrollToShot(i);"
                    >
                        <img :src="getPageUrl(i, displayedVersions[0])" loading="lazy" class="w-full h-full object-cover opacity-70 group-hover:opacity-100 transition-opacity" />
                        ...
                        <div v-if="i === shotNumber" class="absolute top-1 right-1 w-1.5 h-1.5 rounded-full bg-blue-500 shadow-sm shadow-blue-500/50"></div>
                    </div>
                </div>
            </div>
            ...
            -->
            <!-- COMPARE MODE: Side-by-side comparison (VERSION COMPARISON) -->
            <div v-if="viewMode === 'list' && isCompareMode" id="compare-container" class="w-full h-full flex overflow-hidden">
                <template v-for="(version, idx) in displayedVersions" :key="version.id">
                    <div 
                    ...
                    >
                        ...
                        <div class="w-full h-full flex items-center justify-center p-4">
                             <img :src="getPageUrl(shotNumber, version)" 
                    ...
                    >
                        ...
                        <div class="h-full w-[1px] bg-white/10 group-hover/handle:bg-blue-500/50 transition-colors"></div>
                        ...
                        <div class="absolute w-4 h-8 bg-black/60 border border-white/20 rounded-full flex flex-col items-center justify-center gap-0.5 opacity-0 group-hover/handle:opacity-100 transition-opacity backdrop-blur hover:bg-black/80 hover:border-blue-500/50">
                            <div class="w-0.5 h-0.5 rounded-full bg-white/70"></div>
                            <div class="w-0.5 h-0.5 rounded-full bg-white/70"></div>
                            <div class="w-0.5 h-0.5 rounded-full bg-white/70"></div>
                        </div>
                    </div>
            ...
            <div v-else-if="viewMode === 'list'" class="w-full h-full overflow-y-auto custom-scrollbar">
                <div class="flex flex-col items-center gap-6 py-8 px-4 md:px-0 pt-20 pb-20">
                    <div v-for="i in 15" :key="i" :id="`shot-${i}`" class="w-full max-w-3xl relative group shadow-2xl bg-black">
                        <img :src="getPageUrl(i, displayedVersions[0])" loading="lazy" class="w-full h-auto object-contain transition-opacity opacity-90 group-hover:opacity-100" />
        ...
        <header class="absolute top-0 left-0 right-0 z-10 h-12 px-4 flex items-center justify-between gap-4 transition-all bg-gradient-to-b from-black/60 to-transparent">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="flex items-center bg-white/5 rounded-full p-1 border border-white/5 backdrop-blur-sm">
                    ...
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-rectangle-horizontal"><rect width="20" height="12" x="2" y="6" rx="2"/></svg>
                    </button>
        ...
        <footer class="absolute bottom-0 left-0 right-0 z-10 h-12 w-full bg-gradient-to-t from-black/80 to-transparent flex items-center px-4 gap-4 transition-all">
             ...
             -->
             <div class="flex-1 flex gap-1.5 md:gap-2 overflow-x-auto items-center custom-scrollbar h-full py-0.5 justify-start md:justify-center" dir="ltr">
                <div v-for="i in filmstripCount" :key="i" 
