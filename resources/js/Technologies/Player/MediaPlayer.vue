...
<template>
  ...
  >
    ...
    <div
      v-if="type === 'audio'"
      class="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-gray-900 via-gray-900 to-black overflow-hidden"
    >
      <!-- Background Image Blur -->
      <div
        v-if="poster"
        class="absolute inset-0 opacity-20 bg-cover bg-center blur-md scale-110"
    ...
    <div
      v-if="isWaiting"
      class="absolute inset-0 z-30 flex items-center justify-center bg-black/40 backdrop-blur-sm pointer-events-none"
    >
      <div class="w-14 h-14 border-4 border-primary-500 border-t-transparent rounded-full animate-spin" />
    </div>
    ...
    >
      <div
        v-if="activeSegment && showControls"
      ...
      >
        <div class="flex items-center gap-3 bg-black/60 backdrop-blur-md px-5 py-2.5 rounded-full border border-gray-700/50 shadow-xl">
          <span
            class="w-2.5 h-2.5 rounded-full animate-pulse shadow-[0_0_10px_currentColor]"
            :style="{ color: activeSegment.color || '#3b82f6', backgroundColor: activeSegment.color || '#3b82f6' }"
          />
    ...
    >
      ...
      >
        <!-- Buffer Bar -->
        <div
          class="absolute top-0 left-0 h-full bg-gray-500/30 rounded-full transition-all duration-500"
        ...
        >
          ...
          <div class="absolute left-0 top-0 bottom-0 w-[1px] bg-white/40" />
          <div class="absolute right-0 top-0 bottom-0 w-[1px] bg-white/40" />
        </div>

        <!-- A-B Loop Region Layer -->
        <div
          v-if="loopRange.start !== null" 
        ...
        >
          <!-- Start Flag -->
          <div class="absolute -top-3 -left-1.5 text-[10px] text-yellow-400 font-bold">
            A
          </div>
          <!-- End Flag -->
          <div
            v-if="loopRange.end"
            class="absolute -top-3 -right-1.5 text-[10px] text-yellow-400 font-bold"
          >
            B
          </div>
        </div>
        ...
        <!-- Play Progress Bar -->
        <div
          class="absolute top-0 left-0 h-full bg-gradient-to-r from-primary-600 to-primary-400 rounded-full flex items-center justify-end z-20" 
          :style="{ width: `${(currentTime / duration) * 100}%` }"
        >
          <!-- Thumb (Glows on Hover) -->
          <div class="w-4 h-4 bg-white rounded-full shadow-[0_0_10px_rgba(255,255,255,0.5)] scale-0 group-hover/timeline:scale-100 transition-transform duration-200" />
        </div>
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
          <!-- Play/Pause -->
          <button
            class="text-white hover:text-primary-400 hover:scale-110 transition-all active:scale-95"
            @click="togglePlay"
          >
            <component
          ...
          <div class="flex items-center gap-1">
            <button
              class="group p-2 rounded-full hover:bg-white/10 text-gray-300 hover:text-white transition-all"
              title="Back 10s"
              @click="skip(-10)"
            >
          ...
          <div class="group/vol flex items-center gap-2 relative pl-2">
            <button
              ...
              @click="setVolume(isMuted ? 1 : 0)"
            >
              <component
                :is="isMuted || volume === 0 ? SpeakerXMarkIcon : SpeakerWaveIcon"
                class="w-6 h-6"
              />
            </button>
