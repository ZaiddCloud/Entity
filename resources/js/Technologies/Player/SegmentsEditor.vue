...
<template>
  <div
    class="flex flex-col h-full bg-gray-900 border-r border-gray-800 text-gray-100 font-sans"
    dir="rtl"
  >
    <div class="p-4 border-b border-gray-800 bg-gray-900/95 backdrop-blur flex justify-between items-center sticky top-0 z-10">
      <h2 class="text-lg font-bold flex items-center gap-2">
        <TagIcon class="w-5 h-5 text-primary-500" />محرر المقاطع
      </h2>
      <button
        class="text-xs bg-primary-600 hover:bg-primary-500 text-white px-3 py-1.5 rounded-lg flex items-center gap-1 transition-all shadow-lg hover:shadow-primary-500/20"
        @click="$emit('save-final', localSegments)"
      >
        <ArrowDownOnSquareIcon class="w-4 h-4" />حفظ العمل
      </button>
    </div>
    ...
    <div class="p-4 border-b border-gray-800 bg-gray-800/30 space-y-4">
      <div class="grid grid-cols-2 gap-4">
        <div class="space-y-1">
          <div class="flex justify-between items-center">
            <label class="text-xs text-gray-400">البداية</label><button
              class="text-[10px] bg-gray-700 hover:bg-white hover:text-black px-1.5 py-0.5 rounded transition-colors flex items-center gap-1"
              @click="captureTime('start')"
            >
              <ClockIcon class="w-3 h-3" />التقاط
            </button>
          </div>
          <input
            ...
            type="number"
            step="0.1"
            class="w-full bg-gray-900 border border-gray-700 rounded px-2 py-1.5 text-sm text-left font-mono focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none"
          >
        </div>
        <div class="space-y-1">
          <div class="flex justify-between items-center">
            <label class="text-xs text-gray-400">النهاية</label><button
              class="text-[10px] bg-gray-700 hover:bg-white hover:text-black px-1.5 py-0.5 rounded transition-colors flex items-center gap-1"
              @click="captureTime('end')"
            >
              <ClockIcon class="w-3 h-3" />التقاط
            </button>
          </div>
      ...
      <div class="space-y-1">
        ...
        <input
          v-model="draft.label"
          type="text"
          placeholder="مثال: المقدمة، شرح الحديث..."
          class="w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-sm focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none placeholder-gray-600"
      ...
      <div class="flex gap-2 justify-between">
        <button
          ...
          :class="draft.color === color.hex ? 'border-white scale-110 shadow-lg' : 'border-transparent opacity-60 hover:opacity-100'"
          :style="{ backgroundColor: color.hex }"
          :title="color.name"
          @click="draft.color = color.hex"
        />
      </div>
      <div class="flex gap-2 pt-2">
        <button
          class="flex-1 bg-primary-600 hover:bg-primary-500 text-white py-2 rounded-lg text-sm font-medium transition-colors flex justify-center items-center gap-2"
          @click="saveDraft"
        >
          <component
            :is="editingIndex !== null ? CheckIcon : PlusIcon"
            class="w-4 h-4"
          />{{ editingIndex !== null ? 'تحديث المقطع' : 'إضافة مقطع' }}
        </button>
        <button
          v-if="editingIndex !== null"
          class="px-3 bg-gray-700 hover:bg-gray-600 rounded-lg text-gray-300 transition-colors"
          title="إلغاء التعديل"
          @click="resetDraft"
        >
    ...
    <div class="flex-1 overflow-y-auto p-3 space-y-2 scrollbar-thin scrollbar-track-gray-900 scrollbar-thumb-gray-700">
      <div
        v-if="localSegments.length === 0"
        class="text-center py-8 text-gray-500 text-sm border-2 border-dashed border-gray-800 rounded-xl"
      >
        لا توجد مقاطع مضافة.<br>استخدم أزرار الالتقاط للبدء.
      </div>
      <div
        v-for="(seg, idx) in localSegments"
        :key="idx"
        class="group relative bg-gray-800 border border-gray-700 hover:border-gray-500 rounded-lg p-3 transition-all cursor-pointer"
        :class="{'ring-1 ring-primary-500 bg-gray-800/80': editingIndex === idx, 'opacity-100 border-l-4': true}"
        :style="{ borderLeftColor: seg.color }"
        @click="emit('seek', seg.start)"
      >
        <div
          v-if="currentTime >= seg.start && currentTime <= seg.end"
          class="absolute inset-0 bg-primary-500/5 pointer-events-none rounded-lg border border-primary-500/20"
        />
        <div class="flex justify-between items-start mb-1 relative z-10">
          <h4 class="font-medium text-sm text-gray-200 line-clamp-1">
            {{ seg.label }}
          </h4>
