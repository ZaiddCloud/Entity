    ...
    <AuthenticatedLayout :title="audio.title">
                    ...
                    <div class="w-full lg:w-auto flex-shrink-0 flex justify-center lg:justify-start">
                         <div class="relative group">
                            ...
                            <div class="relative w-48 h-48 lg:w-64 lg:h-64 rounded-2xl overflow-hidden shadow-2xl bg-emerald-900 border-4 border-emerald-900/50 flex items-center justify-center">
                                ...
                                >
                                <div v-else class="text-emerald-700/50 flex flex-col items-center">
                                     <svg class="w-20 h-20 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z"></path></svg>
                    ...
                    <div class="flex-1 text-center lg:text-right space-y-6">
                        <div class="space-y-2">
                             <div class="flex flex-wrap items-center justify-center lg:justify-start gap-4 text-xs font-bold uppercase tracking-wider text-emerald-400/80 mb-2">
                                ...
                                <span v-if="audio.versions?.[0]?.published_year" class="bg-emerald-900/50 px-3 py-1 rounded-full border border-emerald-500/20 backdrop-blur-sm">
                            ...
                            <p v-if="audio.description" class="text-lg text-emerald-100/70 max-w-3xl mx-auto lg:mx-0 leading-relaxed line-clamp-2">
                                {{ audio.description }}
                            </p>
                        </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-10 pb-20">
            ...
            <div class="space-y-8">
                        ...
                        <div class="lg:col-span-2 space-y-8">
                            ...
                            <Card class="!p-8">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div>
                                        <h3 class="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">المؤلف / القارئ</h3>
                                        <div class="flex flex-wrap gap-2">
                                            <div v-if="audio.authors?.length" class="flex flex-wrap gap-2">
                                                 <Link 
                                            ...
                                            <span v-else class="text-sm text-gray-400 italic">غير محدد</span>
                                        </div>
                                    </div>
                                    <div>
                                        <h3 class="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">الناشر / المنتج</h3>
                                        <div v-if="audio.versions?.[0]?.publisher" class="flex items-center gap-3">
                                ...
                                <div class="mt-8 pt-8 border-t border-gray-100 dark:border-white/5 grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div>
                                         <h3 class="text-xs font-black text-gray-400 uppercase tracking-widest mb-3">التصنيفات</h3>
                                        <div class="flex flex-wrap gap-2">
                                            <Badge v-for="cat in audio.categories" :key="cat.id" color="emerald" size="md">
                                                {{ cat.name }}
                                            </Badge>
                                    ...
                                    <div>
                                        <h3 class="text-xs font-black text-gray-400 uppercase tracking-widest mb-3">الوسوم</h3>
                                        <div class="flex flex-wrap gap-2">
                                            <Badge v-for="tag in audio.tags" :key="tag.id" color="gray" size="sm">
                                                #{{ tag.name }}
                                            </Badge>
                            ...
                            <Card class="!p-8">
                                <h3 class="text-lg font-black text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                                    <svg class="w-5 h-5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h7" /></svg>
                                    الوصف الكامل
                                </h3>
                                <div class="prose prose-lg dark:prose-invert max-w-none prose-p:text-gray-600 dark:prose-p:text-gray-300">
                                    <p v-if="audio.description" class="whitespace-pre-line">{{ audio.description }}</p>
                                    <p v-else class="italic text-gray-400 text-sm">لا يوجد وصف متاح لهذا التسجيل.</p>
                                </div>
                            </Card>
                        </div>
                        ...
                        <div class="lg:col-span-1 space-y-8">
                            <div class="grid grid-cols-2 lg:grid-cols-1 gap-4">
                                <Card class="!p-6 !bg-emerald-500/5 group hover:!bg-emerald-500/10 transition-colors border-emerald-500/10">
                                    ...
                                    <div class="text-xs font-bold text-gray-500 uppercase tracking-wider">المدة (ثواني)</div>
                                </Card>
                                ...
                                <Card class="!p-6 group hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                                    ...
                                    <div class="text-xs font-bold text-gray-500 uppercase tracking-wider">سنة الإصدار</div>
                                </Card>
                            </div>
                ...
                <!-- Content Tab -->
                <div v-show="activeTab === 'content'" class="space-y-8">
                     <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <Card class="!p-8 h-full flex flex-col justify-center items-center text-center">
                            <div class="w-20 h-20 rounded-full bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center text-emerald-500 mb-6 group-hover:scale-110 transition-transform">
                                <svg class="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg>
                            </div>
                            <h3 class="text-xl font-black text-gray-900 dark:text-white mb-2">الاستماع للتسجيل</h3>
                            <p class="text-gray-500 text-sm max-w-sm mb-8">استمع للملف الصوتي مباشرة</p>
                            ...
                            >
                             ...
                             <div v-else class="py-4 text-amber-500 font-bold bg-amber-50 dark:bg-amber-500/10 px-6 rounded-xl text-sm">
                        ...
                        <Card class="!p-8 h-full flex flex-col justify-center items-center text-center">
                            <div class="w-20 h-20 rounded-full bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-blue-500 mb-6 group-hover:scale-110 transition-transform">
                                <svg class="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                            </div>
                            <h3 class="text-xl font-black text-gray-900 dark:text-white mb-2">المحتوى النصي</h3>
                            <p class="text-gray-500 text-sm max-w-sm mb-8">تحرير النصوص، التفريغ، والهوامش عبر المحرر المتقدم</p>
                            <Link 
                                v-if="first_content_slug"
                                :href="route('editor.show', { type: 'audio', slug: first_content_slug })"
                            >
                             ...
                             <div v-else class="py-2 text-gray-400 text-sm">
                ...
                <div v-show="activeTab === 'details'" class="space-y-8">
                     <Card class="!p-0 overflow-hidden">
                        <div class="p-8 border-b border-gray-100 dark:border-white/5">
                            <h3 class="text-lg font-black text-gray-900 dark:text-white">تفاصيل إضافية</h3>
                        </div>
                        <div class="divide-y divide-gray-100 dark:divide-white/5">
                            <div class="grid grid-cols-1 md:grid-cols-3 p-6 hover:bg-gray-50/50 dark:hover:bg-white/2 transition-colors">
                                <div class="text-sm font-bold text-gray-500">الرقم التسلسلي</div>
                                <div class="md:col-span-2 text-sm font-mono font-bold text-gray-900 dark:text-white">{{ audio.formatted_serial_number }}</div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-3 p-6 hover:bg-gray-50/50 dark:hover:bg-white/2 transition-colors">
                                <div class="text-sm font-bold text-gray-500">العنوان الأصلي</div>
                                <div class="md:col-span-2 text-sm text-gray-900 dark:text-white">{{ audio.title }}</div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-3 p-6 hover:bg-gray-50/50 dark:hover:bg-white/2 transition-colors">
                                <div class="text-sm font-bold text-gray-500">تاريخ الإنشاء</div>
                                <div class="md:col-span-2 text-sm font-mono text-gray-900 dark:text-white">{{ new Date(audio.created_at).toLocaleString('ar-EG') }}</div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-3 p-6 hover:bg-gray-50/50 dark:hover:bg-white/2 transition-colors">
                                <div class="text-sm font-bold text-gray-500">آخر تحديث</div>
                                <div class="md:col-span-2 text-sm font-mono text-gray-900 dark:text-white">{{ new Date(audio.updated_at).toLocaleString('ar-EG') }}</div>
                            </div>
                        </div>
                    </Card>
                </div>
