      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-xs text-gray-400 font-bold mt-1">
            عرض كافة النتائج المطابقة لطلباتك عبر كافة أقسام المكتبة
          </p>
        </div>
      </div>
    </template>
    ...
    <div class="space-y-12">
      ...
      >
        ...
        >
          ...
          <div class="space-y-3">
            <Link
              ...
              class="block bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-white/5 rounded-2xl p-4 hover:border-indigo-500/50 hover:shadow-lg transition-all"
            >
        ...
        >
          ...
          <div class="space-y-3">
            <Link
              v-for="item in results.authors"
              :key="item.id"
              :href="route('authors.show', item.id)"
              class="block bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-white/5 rounded-2xl p-4 hover:border-teal-500/50 hover:shadow-lg transition-all"
            >
              <div class="text-sm font-black dark:text-white">
                {{ item.name }}
              </div>
        ...
        <div
          v-if="results.audios?.length || results.videos?.length"
          class="space-y-4"
        >
          <h3 class="flex items-center gap-2 text-sm font-black text-gray-400 uppercase tracking-widest px-4">
            <svg
          ...
          <div class="space-y-3">
            <Link
              v-for="item in results.audios"
              :key="'audio-'+item.id"
              :href="route('audios.show', item.id)"
              class="block bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-white/5 rounded-2xl p-4 hover:border-sky-500/50 hover:shadow-lg transition-all"
            >
            ...
            <Link
              v-for="item in results.videos"
              :key="'video-'+item.id"
              :href="route('videos.show', item.id)"
              class="block bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-white/5 rounded-2xl p-4 hover:border-purple-500/50 hover:shadow-lg transition-all"
            >
              <div class="flex justify-between items-center">
                <div class="text-sm font-black dark:text-white">
                  {{ item.title }}
      ...
      >
        ...
        <h3 class="text-xl font-black dark:text-white mb-2">
