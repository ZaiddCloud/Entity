                ...
                <div class="flex flex-col md:flex-row gap-8 items-center md:items-end">
                    ...
                    <div class="w-48 h-48 rounded-[2.5rem] bg-white dark:bg-black/40 p-1 shadow-2xl relative group">
                        ...
                        <div class="w-full h-full rounded-[2.2rem] bg-gray-50 dark:bg-emerald-900/20 overflow-hidden flex items-center justify-center border border-white/10 relative z-10">
                            <svg class="w-24 h-24 text-emerald-500/40" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16" /></svg>
                        </div>
                    </div>
                    ...
                    <div class="flex-1 text-center md:text-right pb-4">
                        <div class="flex items-center justify-center md:justify-start gap-2 mb-2">
                            <Badge variant="gray" class="!bg-emerald-500/10 !text-emerald-400 !border-emerald-500/20 !text-[10px] font-black uppercase tracking-wider backdrop-blur-sm">سلسلة علمية</Badge>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <Card class="!p-6">
                         <h4 class="font-black text-sm mb-4 text-emerald-600 dark:text-emerald-400 flex items-center justify-between">
                            الكتب
                            <Badge color="emerald" class="!text-[10px]">{{ series.books.length }}</Badge>
                        </h4>
                        <ul class="space-y-3">
                            <li v-for="book in series.books" :key="book.id" class="group">
                            ...
                            <li v-if="!series.books.length" class="text-xs text-gray-400 italic">لا توجد كتب مضافة</li>
                        </ul>
                    </Card>
                    ...
                    <Card class="!p-6">
                         <h4 class="font-black text-sm mb-4 text-blue-600 dark:text-blue-400 flex items-center justify-between">
                            الصوتيات
                            <Badge color="blue" class="!text-[10px]">{{ series.audio.length }}</Badge>
                        </h4>
                        <ul class="space-y-3">
                            <li v-for="item in series.audio" :key="item.id" class="group">
                            ...
                            <li v-if="!series.audio.length" class="text-xs text-gray-400 italic">لا توجد صوتيات</li>
                        </ul>
                    </Card>
                    ...
                    <Card class="!p-6">
                         <h4 class="font-black text-sm mb-4 text-purple-600 dark:text-purple-400 flex items-center justify-between">
                            المرئيات
                            <Badge color="purple" class="!text-[10px]">{{ series.videos.length }}</Badge>
                        </h4>
                        <ul class="space-y-3">
                            <li v-for="item in series.videos" :key="item.id" class="group">
                            ...
                            <li v-if="!series.videos.length" class="text-xs text-gray-400 italic">لا توجد مرئيات</li>
                        </ul>
                    </Card>
                    ...
                    <Card class="!p-6">
                         <h4 class="font-black text-sm mb-4 text-amber-600 dark:text-amber-400 flex items-center justify-between">
                            المخطوطات
                            <Badge color="amber" class="!text-[10px]">{{ series.manuscripts.length }}</Badge>
                        </h4>
                        <ul class="space-y-3">
                            <li v-for="item in series.manuscripts" :key="item.id" class="group">
                            ...
                            <li v-if="!series.manuscripts.length" class="text-xs text-gray-400 italic">لا توجد مخطوطات</li>
                        </ul>
                    </Card>
                </div>
            </div>
