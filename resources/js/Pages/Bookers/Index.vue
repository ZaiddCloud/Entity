                        ...
                        <tbody class="divide-y divide-gray-50 dark:divide-white/5">
                            <tr v-for="booker in bookers.data" :key="booker.id" class="group hover:bg-gray-50/50 dark:hover:bg-white/2 transition-colors">
                                <td class="px-8 py-6 whitespace-nowrap">
                                    <input type="checkbox" :value="booker.id" v-model="selectedIds" class="rounded-lg border-gray-300 dark:border-white/10 dark:bg-black text-indigo-600 shadow-sm focus:ring-indigo-500" />
                                </td>
                                <td class="px-8 py-6 whitespace-nowrap">
...
watch(search, debounce((value) => {
    router.get(route('bookers.index'), { search: value }, {
        preserveState: true,
        replace: true,
    });
}, 300));
...
const allSelected = computed({
    get: () => props.bookers.data.length > 0 && selectedIds.value.length === props.bookers.data.length,
    set: (value) => {
        selectedIds.value = value ? props.bookers.data.map(b => b.id) : [];
    }
});

const bulkDelete = () => {
    if (confirm('هل أنت متأكد من حذف المساهمين المحددين؟')) {
        router.post(route('bookers.bulk-destroy'), {
