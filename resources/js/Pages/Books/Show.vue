                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                            ...
                            <div>
                                ...
                                <div>
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        <span v-for="tag in book.tags" :key="tag.id" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                            {{ tag.name }}
                                        </span>
                                        <span v-if="!book.tags.length" class="text-sm text-gray-400italic">لا يوجد وسوم</span>
                                    </div>
                                </div>
                                <div class="mt-6">
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">التصنيفات</dt>
                                    <div class="flex flex-wrap gap-2">
                                        <span v-for="category in book.categories" :key="category.id" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                            {{ category.name }}
                                        </span>
                                        <span v-if="!book.categories.length" class="text-sm text-gray-400 italic">لا يوجد تصنيفات</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-10 border-t border-gray-200 dark:border-gray-700 pt-8">
                            ...
                            <div class="space-y-4">
                                <div v-for="comment in book.comments" :key="comment.id" class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                                ...
                                <p v-if="!book.comments.length" class="text-sm text-gray-400 italic">لا يوجد تعليقات بعد</p>
                            </div>
                        </div>
                    </div>
                </div>
