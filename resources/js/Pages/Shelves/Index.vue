        ...
        <template #header>
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="font-black text-2xl dark:text-white leading-tight text-emerald-600">الرفوف والخزائن</h2>
                        <p class="text-xs text-gray-400 font-bold mt-1 uppercase tracking-widest">تنظيم وتتبع المواقع المادية والرقمية للمحتوى</p>
                    </div>
                </div>
            ...
            <Table>
                <template #head>
                    <TableHead>
                        ...
                        <TableHeaderCell class="text-center">السعة الاستيعابية</TableHeaderCell>
                ...
                <template #body>
                    <TableBody>
                        <TableRow v-for="shelf in shelves.data" :key="shelf.id">
                            <TableCell>
                                <input type="checkbox" v-model="selectedIds" :value="shelf.id" class="rounded border-gray-300 text-emerald-600 shadow-sm focus:ring-emerald-500 dark:bg-white/5 dark:border-white/10" />
                            </TableCell>
                            <TableCell>
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="font-black text-gray-900 dark:text-white">{{ shelf.location_code }}</div>
                                        <div class="text-[10px] text-gray-400 font-bold mt-1 uppercase tracking-tighter line-clamp-1 max-w-sm">ID: {{ shelf.id.substring(0, 8) }}...</div>
                                    </div>
                                </div>
                            </TableCell>
                            ...
                            <TableCell class="text-left">
                                <div class="flex items-center justify-end gap-2">
                                    <IconButton :href="route('shelves.show', shelf.id)" color="emerald" title="عرض">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                    </IconButton>
                                    <IconButton :href="route('shelves.edit', shelf.id)" color="blue" title="تعديل">
