<template>
    <AuthenticatedLayout title="تعديل الرف">
        <template #header>
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="font-black text-2xl dark:text-white leading-tight text-blue-600">تعديل الرف</h2>
                    <p class="text-[10px] text-gray-400 font-bold mt-1 uppercase tracking-widest">تحديث بيانات الموقع: {{ shelf.location_code }}</p>
                </div>
            </div>
        </template>
        ...
        <div class="max-w-4xl mx-auto">
            <Card>
                <form @submit.prevent="form.put(route('shelves.update', shelf.id))" class="space-y-8">
...
const form = useForm({
    location_code: props.shelf.location_code,
