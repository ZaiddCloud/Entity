<template>
  <AuthenticatedLayout :title="'رف: ' + shelf.location_code">
    ...
    <div class="space-y-12 py-8">
      ...
      <div class="flex items-center gap-8 border-b border-gray-100 dark:border-white/5 pb-px overflow-x-auto no-scrollbar">
        ...
        >
          ...
          <div
            ...
            class="absolute bottom-0 left-0 right-0 h-1 bg-emerald-500 rounded-full"
          />
        </button>
        <button
          :class="[
      ...
      >
        <div
          v-if="shelf.versions?.length > 0"
          class="grid grid-cols-1 md:grid-cols-2 gap-6"
        >
          <Card
            v-for="version in shelf.versions"
            :key="version.id"
            class="group"
          >
            <div class="flex items-start gap-4">
              <div class="w-12 h-12 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center text-emerald-600 shrink-0 border border-emerald-100 dark:border-emerald-500/20">
                <svg
                  v-if="version.versionable_type.includes('Book')"
                  class="w-6 h-6"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                ...
                /></svg>
                <svg
                  v-else-if="version.versionable_type.includes('Audio')"
                  class="w-6 h-6"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                ...
                /></svg>
                <svg
                  v-else-if="version.versionable_type.includes('Video')"
                  class="w-6 h-6"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1">
                  <Badge color="emerald">
                    {{ version.version_label || 'نسخة أساسية' }}
                  </Badge>
        ...
        <div
          v-else
          class="py-20 text-center bg-gray-50 dark:bg-white/2 rounded-[2.5rem] border border-dashed border-gray-200 dark:border-white/10"
        >
          <div class="w-16 h-16 bg-white dark:bg-white/5 rounded-2xl flex items-center justify-center text-gray-300 mx-auto mb-4">
            ...
            ><path
              ...
              stroke-linejoin="round"
              stroke-width="2"
              d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 00-2 2H6a2 2 0 00-2 2v-5m16 0h-3.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"
            /></svg>
          </div>
          <p class="text-gray-400 font-bold text-xs uppercase tracking-widest">
            هذا الرف خالٍ حالياً
          </p>
        </div>
      </div>
      ...
      >
        <Card class="bg-emerald-50/50 dark:bg-emerald-500/5 border-emerald-100 dark:border-emerald-500/10">
          <div class="flex flex-col items-center text-center p-4">
            <div class="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-[0.2em] mb-4">
              نسبة الإشغال
            </div>
        ...
        <Card>
          <div class="flex flex-col items-center text-center p-4">
            <div class="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4">
              المساحة المتاحة
            </div>
