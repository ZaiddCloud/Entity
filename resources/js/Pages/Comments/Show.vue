...
<template>
    <AuthenticatedLayout title="عرض تعليق">
        <template #header>
                    ...
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <div class="grid grid-cols-2 gap-4 border-t border-gray-200 dark:border-gray-700 pt-4">
                            <div>
                                ...
                                <p class="font-medium">{{ comment.user?.name || 'غير معروف' }}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-500 dark:text-gray-400">تاريخ الإنشاء</p>
                                <p class="font-medium">{{ new Date(comment.created_at).toLocaleString('ar-EG') }}</p>
                            </div>
                        </div>

                        <div class="mt-6">
                            <Link :href="route('comments.index')" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400">
                                ← العودة إلى القائمة
                            </Link>
                        </div>
                    </div>
