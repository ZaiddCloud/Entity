        ...
        <template #header>
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="font-black text-2xl dark:text-white leading-tight">المكتبة الصوتية</h2>
                        <p class="text-xs text-gray-400 font-bold mt-1">تسجيلات المحاضرات، الخطب، والكتب المسموعة</p>
                    </div>
                </div>
                <Link
                    ...
                    <table class="min-w-full divide-y divide-gray-100 dark:divide-white/5">
                        <thead class="bg-gray-50/50 dark:bg-white/2">
                            <tr>
                                ...
                                <th scope="col" class="px-8 py-5 text-right text-[10px] font-black text-gray-400 uppercase tracking-widest">عنوان التسجيل</th>
                                <th scope="col" class="px-8 py-5 text-right text-[10px] font-black text-gray-400 uppercase tracking-widest">المؤلف / القارئ</th>
                                <th scope="col" class="px-8 py-5 text-right text-[10px] font-black text-gray-400 uppercase tracking-widest">الناشر / المنصة</th>
                        ...
                        <tbody class="divide-y divide-gray-50 dark:divide-white/5">
                            <tr v-for="audio in audios.data" :key="audio.id" class="group hover:bg-gray-50/50 dark:hover:bg-white/2 transition-colors">
                                <td class="px-8 py-6 whitespace-nowrap text-sm font-black font-mono text-gray-300 dark:text-gray-600 group-hover:text-rose-500 transition-colors">
                                    {{ audio.formatted_serial_number }}
                                </td>
                                <td class="px-8 py-6 whitespace-nowrap">
                                    <div class="text-sm font-black text-gray-900 dark:text-white">{{ audio.title }}</div>
                                    <div class="flex gap-1 mt-1">
                                        <span v-for="tag in audio.tags" :key="tag.id" class="text-[9px] font-black text-gray-400 uppercase">#{{ tag.name }}</span>
                                    </div>
                                </td>
                                <td class="px-8 py-6 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                    {{ audio.authors?.map(a => a.name).join('، ') || '-' }}
                                </td>
                                <td class="px-8 py-6 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 font-bold uppercase tracking-tighter">
                                    {{ audio.versions?.[0]?.publisher?.name || 'إنتاج داخلي' }}
                                </td>
                                <td class="px-8 py-6 whitespace-nowrap text-left text-sm font-medium">
                                    <div class="flex items-center justify-end gap-2">
                                        <Link :href="route('audios.show', audio.slug)" class="p-2 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 rounded-xl transition-all" title="عرض">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                        </Link>
                                        <Link :href="route('audios.edit', audio.slug)" class="p-2 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 rounded-xl transition-all" title="تعديل">
