            ...
            <Table>
                <TableHead>
                    <TableHeaderCell class="!px-4 w-12 text-center">
                        <input id="authors-select-all" type="checkbox" v-model="allSelected" class="rounded border-gray-300 dark:border-white/10 dark:bg-black text-emerald-600 focus:ring-emerald-500" />
                    </TableHeaderCell>
                ...
                <TableBody>
                    <TableRow v-for="author in authors.data" :key="author.id">
                        <TableCell class="!px-4 text-center">
                            <input :id="'author-select-' + author.id" type="checkbox" :value="author.id" v-model="selectedIds" class="rounded border-gray-300 dark:border-white/10 dark:bg-black text-emerald-600 focus:ring-emerald-500" />
                        </TableCell>
                        <TableCell>
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="font-black text-gray-900 dark:text-white">{{ author.name }}</div>
                                    <div class="text-[10px] text-gray-400 font-bold mt-1 uppercase tracking-tighter">سنة الميلاد: {{ author.birth_year || '-' }}</div>
                                </div>
                            </div>
                        </TableCell>
                        <TableCell>
                            <Badge v-if="author.death_year" variant="error" class="!bg-rose-50 !text-rose-600 !border-rose-100">
                                توفى عام {{ author.death_year }} هـ
                            </Badge>
                            <Badge v-else variant="success" class="!bg-emerald-50 !text-emerald-600 !border-emerald-100">
                                على قيد الحياة
                            </Badge>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell class="text-left">
                            <div class="flex items-center justify-end gap-2">
                                <IconButton :href="route('authors.show', author.id)" color="emerald" title="عرض التفاصيل">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                </IconButton>
                                <IconButton :href="route('authors.edit', author.id)" color="blue" title="تعديل">
